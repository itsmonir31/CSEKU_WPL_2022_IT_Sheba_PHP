          ...
          <div class="col-1-3 col-1-3-sm">
            <div class="controls">
              ...
              <select class="floatLabel" name="prblm_type_" required="">
                <?php 
                  $sql="SELECT * FROM prblm_types ORDER BY prblm_type ASC";
                  $qry = mysqli_query($conn,$sql);

                  while ( $rows = mysqli_fetch_assoc($qry)){?>
                <option value="<?php echo $rows["prblm_type"];?>">
          ...
          <div class="col-1-3 col-1-3-sm">
            <div class="controls">
              ...
              <select class="floatLabel" name="dev_name_" required="">
                <?php 
                  $sql="SELECT * FROM dev_types ORDER BY dev_name ASC";
                  $qry = mysqli_query($conn,$sql);

                  while ( $rows = mysqli_fetch_assoc($qry)){?>
                <option value="<?php echo $rows["dev_name"];?>">
                  <?php echo $rows["dev_name"]; ?>
                </option>
                <?php 
                  }
                ?>
          ...
          <div class="col-2-3">
            <div class="controls">
              ...
              <select class="floatLabel" name="city_name_" required="">
                  <?php 
                    $sql="SELECT * FROM available_city ORDER BY city_name ASC";
                    $qry = mysqli_query($conn,$sql);

                    while ( $rows = mysqli_fetch_assoc($qry)){?>
                  <option value="<?php echo $rows["city_name"];?>">
